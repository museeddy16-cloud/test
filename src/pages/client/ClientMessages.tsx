...
export default function ClientMessages() {
  const { data: conversations, loading } = useFetch<Conversation[]>('/client/conversations');
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    const result = await post(`/client/conversations/${selectedConversation.id}/messages`, {
      content: newMessage,
    });
                  ...
                  >
                    <div className="participant-avatar">
                      {otherParticipant.avatar ? (
                        ...
                        <span>{otherParticipant.firstName?.charAt(0) || 'H'}</span>
                      )}
                    </div>
                  ...
                  <div>
                    <h3>
                      {selectedConversation.participants
                        .find((p) => p.id !== localStorage.getItem('userId'))?.firstName || 'Host'}
                    </h3>
                    <span className="listing-info">
                      {selectedConversation.listingId && 'Regarding booking'}
                    </span>
                  </div>
                </div>
